...
public class Board : Helpers
{
    ...
    public bool wasPressingB = false;
        ...
        String INITIAL_FILE_NAME, double ts)
    {
        ...
        for (int x = 0; x < boardWidth; x++)
        {
            for (int y = 0; y < boardHeight; y++)
            {
                ...
                float climateType = (float) (noise(x*stepSize + 10000, y*stepSize + 10000)*1.63 - 0.4);
    ...
    public void drawUI(float scaleUp, double timeStep, int x1, int y1, int x2, int y2, PFont font)
    {
        ...
        if (selectedCreature == null)
        {
            ...
            {
                float x = (i%2)*230 + 10;
                float y = (float) (Math.Floor(i/2.0)*50 + 570);
                fill(buttonColor);
                rect(x, y, 220, 40);
                if (i >= 2 && i < 6)
                {
        ...
        else
        {
            ...
            pushMatrix();
            translate(400, 80);
            float apX = (float) Math.Round((mouseX - 264 - x1)/26.0);
            float apY = (float) Math.Round((mouseY - 80 - y1)/26.0);
    ...
    {
        int index = (int) (random(0, creatures.Count));
